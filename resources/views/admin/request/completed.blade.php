      ...
      <tbody>
        @foreach($completedRequests as $i => $req)
          <tr class="{{ $i % 2 ? 'bg-gray-50' : 'bg-white' }} hover:bg-gray-100">
            ...
            <td class="px-4 py-2 border border-gray-300 text-green-600 font-semibold">{{ ucfirst($req->status) }}</td>
        ...
        @if($completedRequests->isEmpty())
          <tr>
            <td colspan="7"
                class="text-center py-4 border border-gray-300">No completed requests found.</td>
          </tr>
        @endif
      </tbody>
    </table>
